...
const right: HTMLDivElement = document.getElementById('right') as HTMLDivElement

async function getId(): Promise<number> {
  const queryParams: URLSearchParams = new URLSearchParams({email: email})
  const idResponse: Response = await fetch('https://fwd.innopolis.app/api/hw2?' +
...
async function getImage(): Promise<Image> {
  ...
  const queryParams: URLSearchParams = new URLSearchParams({num: id.toString()})
  const imageResponse: Response = await fetch('https://getxkcd.vercel.app/api/comic?' +
      queryParams.toString())
